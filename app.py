...
TELEGRAM_BOT_TOKEN = "8614031634:AAFsFqVPD4LppMqNRdGLMmgnWOcqE7dSp6c"
TELEGRAM_CHAT_ID = "7321126254"
...
ADMIN_PASSWORD_HASH = hashlib.sha256("cc3m123".encode()).hexdigest()
...
def ban7():
    try:
        ...
        if not is_pro and usage.get('ban7', 0) >= 1:
            return jsonify({
                'success': False,
                'error': 'Bạn đã dùng hết lượt sử dụng miễn phí (1 lần). Nâng cấp Pro hoặc nhập Key để tiếp tục.'
            })
...
def spam_log():
    ...
    try:
        ...
        if action == 'start':
            ...
            usage = get_user_usage(username)
            # Check usage limit (2 uses for free users)
            if not is_pro and usage.get('spam_log', 0) >= 2:
                return jsonify({
                    'success': False,
                    'error': 'Bạn đã dùng hết lượt sử dụng miễn phí (2 lần). Nâng cấp Pro hoặc nhập Key để tiếp tục.'
                })
